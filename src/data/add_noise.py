...
def add_noise(pink, 
    ...
    if beeping:
        beep_file = "src/data/beeping.wav"
    ...
    for input_file in coughing_files:
        ...
        base_audio = AudioSegment.from_file(input_file, format="wav")
        ...
        burst_duration_ms = noise_burst_duration * 1000  # 1 second per burst

        for _ in range(noise_burst_num):
            start_ms = random.randint(0, len(base_audio) - burst_duration_ms)
            noise_type = random.choice(["white"] + (["pink"] if pink else []) + (["brown"] if brown else [])) # white is always there
            noise_segment = generate_noise(burst_duration_ms, base_audio.frame_rate, base_audio.channels, noise_type)
            base_audio = base_audio.overlay(noise_segment, position=start_ms)

        if beeping:
            # Overlay beep sound at the beginning
            beep_duration = min(len(beep_sound), len(base_audio))  # Limit beep to base audio length
        ...
        if talking:
            ...
            talking_file = f"./CoughSegmentation/Data/{random.choice(talking_files)}"
            print(talking_file)
            talking_sound = AudioSegment.from_file(talking_file, format="wav")
            talking_duration = min(len(talking_sound), len(base_audio))
            talking_trimmed = talking_sound[:talking_duration]  # Trim beep if needed
        ...
        print(f"Processed audio with noise (white, pink, brown) and beeps saved to '{output_file}'")
...
parser.add_argument("--white_level", type=str, help="Level of white noise", default=0.01)
parser.add_argument("--pink_level", type=str, help="Level of pink noise", default=0.01)
parser.add_argument("--brown_level", type=str, help="Level of brown noise", default=0.01)
parser.add_argument("--noise_burst_num", type=str, help="Number of noise bursts", default=20)
parser.add_argument("--noise_burst_duration", type=str, help="Duration of each noise burst", default=1.0)
parser.add_argument("--beeping", type=str, help="Do you want beeping noise", default=False)
parser.add_argument("--talking", type=str, help="Do you want talking noise", default=False)

args = parser.parse_args()
add_noise(pink=(args.pink == "True"), 
